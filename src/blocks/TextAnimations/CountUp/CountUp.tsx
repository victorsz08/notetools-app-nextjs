...
export default function CountUp({
    ...
    useEffect(() => {
        const unsubscribe = springValue.on("change", (latest) => {
            if (ref.current) {
                const options = {
                    useGrouping: true, // Sempre agrupar milhares
                    minimumFractionDigits: 0,
                    maximumFractionDigits: 0,
                };
    
                let formattedNumber = Intl.NumberFormat("pt-BR", options).format(
                    Number(latest.toFixed(0))
                );
    
                // Se o usuário definir um separator, substituir apenas os separadores de milhar
